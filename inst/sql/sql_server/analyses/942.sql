-- 942 Number of records by drug_concept_id by age decile
INSERT INTO @results_database_schema.@results_table (
SELECT
	942 AS analysis_id,
    de.drug_concept_id AS stratum_1,
    FLOOR((YEAR(de.drug_era_start_date) - p.year_of_birth) / 10) AS stratum_2,
...
WHERE drug_concept_id > 0 -- not in original 404 analysis
GROUP BY
    de.drug_concept_id,
