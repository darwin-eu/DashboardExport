-- 2343 Number of records by episode_concept_id by ICH age group
...
WITH cte1 AS (
  ...
  SELECT
    ...
    DATEDIFF(day, p.date_of_birth, ep.episode_start_date) AS age_days,
    DATEDIFF(year, p.date_of_birth, ep.episode_start_date) AS age_years
  FROM cte1 p
  ...
  WHERE DATEDIFF(year, p.date_of_birth, ep.episode_start_date) < 19
    AND episode_concept_id != 0
), cte3 as (
  SELECT *,
    case
      when age_days < 0
...
select
  2343 as analysis_id,
  episode_concept_id as stratum_1,
...
from cte3
group by episode_concept_id, age_group
;